...
from xuanwu.const import OKEX_SWAP
        ...
        for ch in self._channels:
            if ch == "kline":
                ...
                for symbol in self._symbols:
                    depth_sub = {
                        "op": "subscribe",
                        "args": [f"swap/depth_l2_tbt:{symbol}"]
                    }
...
class OKEXSwapRestApi:
    ...
    async def get_all_ticker(self):
        ...
        :return:
        """
        success, error = await self.request("GET", "/api/swap/v3/instruments/ticker")
    ...
    async def get_contract_info(self, contract_code=None):
        ...
        * 注意: 1. 如果`contract_code`参数不为None, 仅仅匹配contract_code所有信息.
        ...
        """
        success, error = await self.request("GET", "/api/swap/v3/instruments", auth=False)
        if error:
            return None, error
        if contract_code:
            for i in success:
    ...
    async def get_price_limit(self, symbol):
        ...
        Return:
            ...
            :return error: 错误信息, 否则为None None.
        """
        success, error = await self.request("GET", f"/api/swap/v3/instruments/{symbol}/price_limit", auth=False)
        return success, error

    async def get_kline(self, symbol, start='', end='', granularity='60'):
        """ 获取合约信息
                Args：
                ...
                * 注意: 1. 如果`contract_code`参数不为None, 仅仅匹配contract_code所有信息.
                        2. 如果`contract_code`参数为None, 匹配`symbol + contract_type`所有信息.
                """
        uri = f'/api/swap/v3/instruments/{symbol}/candles'
        params = {}
        if start:
            params['start'] = start
        if end:
    ...
    async def get_orderbook(self, symbol, depth=None, limit=10):
        ...
        if depth:
            params["depth"] = depth
        success, error = await self.request("GET", f"/api/swap/v3/instruments/{symbol}/depth", params=params)
    ...
    async def get_position(self, instrument_id):
        """ 获取某个合约的持仓信息.
        Args:
            :param instrument_id: 合约名称ID, e.g. BTC-USD-SWAP.

        Returns:
            :return success: 成功但会否则返回None.
            :return error: 有错误时返回错误信息否则返回None.
        """
        uri = f"/api/swap/v3/{instrument_id}/position"
    ...
    async def create_order(self, symbol, type, price, size, order_type="0", match_price="0", client_oid=None):
        """Create an order.
        Args:
            :param symbol: 合约ID，如BTC-USD-SWAP ,BTC-USDT-SWAP
        ...
        result, error = await self.request("POST", "/api/swap/v3/order", body=order_info, auth=True)
        return result, error
    ...
    async def create_orders(self, symbol, order_datas):
        ...
        success, error = await self.request("POST", "/api/swap/v3/orders", body=params, auth=True)
    ...
    async def revoke_order(self, symbol, order_no):
        ...
        Returns:
            ...
            :return error: 有错误时返回错误信息否则返回None.
        """
        uri = "/api/swap/v3/cancel_order/{symbol}/{order_no}".format(symbol=symbol, order_no=order_no)
    ...
    async def revoke_orders(self, symbol, ids=None, client_oids=None):
        ...
        Returns:
            ...
            :return error: 有错误时返回错误信息否则返回None.
        """
        uri = "/api/swap/v3/cancel_batch_orders/{symbol}".format(symbol=symbol)
        ...
        if ids:
            ...
            order_nos = client_oids
            order_key = "client_oids"

        if len(order_nos) > 10:
            step = 10
            order_list = [order_nos[i:i + step] for i in range(0, len(order_nos), step)]
            for ol in order_list:
                body[order_key] = ol
                result, error = await self.request("POST", uri, body=body, auth=True)
                return result, error
        else:
            body[order_key] = order_nos
            result, error = await self.request("POST", uri, body=body, auth=True)
            return result, error
    ...
    async def get_order_info(self, instrument_id, order_id):
        ...
        """
        uri = "/api/swap/v3/orders/{instrument_id}/{order_id}".format(
    ...
    async def get_funding(self, symbol):
        ...
        """
        uri = f"/api/swap/v3/instruments/{symbol}/funding_time"
...
class OkexSwapTrade:
    ...
    def __init__(self, **kwargs):
        ...
        self._rest_api = OKEXSwapRestApi(
            secret_key=self._secret_key,
            access_key=self._access_key,
            passphrase=self._passphrase
        )
        # Asset detail, {"BTC": {"free": "1.1", "locked": "2.2", "total": "3.3"}, ... }.
        self._assets = Asset(platform=self._platform, account=self._account)
    ...
    async def auth_callback(self, data):
        ...
        data = {
            "op": "subscribe",
            "args": [f"swap/order:{self._symbol}"]
        }
        await self._ws.send(data)
        ...
        data = {
            "op": "subscribe",
            "args": [f"swap/position:{self._symbol}"]
        }
        await self._ws.send(data)
        ...
        data = {
            "op": "subscribe",
            "args": [f"swap/account:{self._symbol}"]
        }
        await self._ws.send(data)
    ...
    async def sub_callback(self, data):
        if not data.get("channel"):
            ...
            logger.error(e, caller=self)
            SingleTask.run(self._error_callback, False, e)
            return
        ...
        if self._subscribe_order_ok and self._subscribe_position_ok and self._subscribe_asset_ok:
            logger.info("初始化账户订单和仓位数据......", caller=self)
            # 初始化订单列表详情
            ...
            if error:
                ...
                success["instrument_id"] = self._symbol
    ...
    async def get_position(self, symbol):
        ...
        :return:
        """
        success, error = await self._rest_api.get_position(instrument_id=symbol)
        if success:
            position = Position(platform="okex_swap", symbol=symbol)
            holding = success["holding"]
            timestamp = tools.utctime_str_to_ms(success["timestamp"])
            """
                'avail_position'    可平数量
                'avg_cost'          开仓平均价
                'instrument_id'     
            ...
            for hold in holding:
                side = hold["side"]
                if side == "long":
                    position.long_avail_qty = hold["avail_position"]
                    position.long_avg_price = hold["avg_cost"]
                    position.long_pos_margin = hold["margin"]
                    position.long_quantity = hold["position"]
                    position.long_pnl = hold["unrealized_pnl"]
                    position.maint_margin_ratio = hold["maint_margin_ratio"]
                    position.liquid_price = hold["liquidation_price"]
                    position.leverage = hold["leverage"]
                    position.created_time = tools.utctime_str_to_ms(hold["timestamp"])
                    position.utime = timestamp

                if side == "short":
                    position.short_avail_qty = hold["avail_position"]
                    position.short_avg_price = hold["avg_cost"]
                    position.short_pos_margin = hold["margin"]
                    position.short_quantity = hold["position"]
                    position.short_pnl = hold["unrealized_pnl"]
    ...
    def _update_position(self, position_info):
        ...
        for position_ in position_info:
            if instrument_id != self._symbol:
                return
            self._position.leverage = position_["leverage"]  # 杠杆倍数
            posi = 0 if position_["position"] == 'null' or position_["position"] is None else position_["position"]
            if position_["side"] == "long":
                self._position.long_quantity = posi  # 多仓数量
                self._position.long_avail_qty = "0" if position_["avail_position"] == "null" or position_["avail_position"] is None else position_["avail_position"]  # 多仓可平仓数量
                self._position.long_avg_price = "0" if position_["avg_cost"] == "null" or position_["avg_cost"] is None else position_["avg_cost"]  # 多仓持仓平均价格
                # self._position.long_pnl_ratio = pnl_ratio  # 多仓收益率
                self._position.long_pnl = "0" if position_["realized_pnl"] == "null" or position_["realized_pnl"] is None else position_["realized_pnl"]  # 多仓盈亏
                self._position.long_pos_margin = "0" if position_["margin"] == "null" or position_["margin"] is None else position_["margin"]  # 多仓持仓保证金

            if position_["side"] == "short":
                self._position.short_quantity = posi  # 空仓数量
                self._position.short_avail_qty = "0" if position_["avail_position"] == "null" or position_["avail_position"] is None else position_["avail_position"]  # 空仓可平仓数量
                self._position.short_avg_price = "0" if position_["avg_cost"] == "null" or position_["avg_cost"] is None else position_["avg_cost"]  # 空仓持仓平均价格
                # self._position.short_pnl_ratio = pnl_ratio  # 空仓收益率
                self._position.short_pnl = "0" if position_["realized_pnl"] == "null" or position_["realized_pnl"] is None else position_["realized_pnl"]  # 空仓盈亏
                self._position.short_pos_margin = "0" if position_["margin"] == "null" or position_["margin"] is None else position_["margin"]  # 空仓持仓保证金
            self._position.liquid_price = "0" if position_["liquidation_price"] == "null" or position_["liquidation_price"] is None else position_["liquidation_price"] # 预估爆仓价格
            self._position.maint_margin_ratio = "0" if position_["maint_margin_ratio"] == "null" or position_["maint_margin_ratio"] is None else position_["maint_margin_ratio"]  # 保证金率
            self._position.created_time = timestamp
            self._position.utime = position_["timestamp"]
