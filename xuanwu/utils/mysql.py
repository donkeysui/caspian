...
class DataBase:
    ...
    def lock(self, tbl_list, lock_type=LOCK_WRITE):
        ...
        if self.debug:
            logger.info(statement, caller=self)
    ...
    def insert(self, entry, table, ignore=False, replace=False):
        ...
        def func2(table, fields, entry_list):
            statement = str.format(f"{'INSERT' if not replace else 'REPLACE'} {'IGNORE ' if ignore else ' '} INTO {table} ({fields}) VALUES {', '.join(map(func, entry_list))}")
            # statement = str.format(f"{'INSERT' if not replace else 'REPLACE'} {'IGNORE ' if ignore else ' '} INTO {table} ({fields}) VALUES {', '.join(map(func, entry_list))}", table, fields, ', '.join(map(func, entry_list)), 'INSERT' if not replace else 'REPLACE', 'IGNORE ' if ignore else ' ')
            if self.debug:
                logger.info(statement, caller=self)
    ...
    def delete(self, table, cond):
        ...
        statement = str.format(f"DELETE FROM  {table} WHERE {conds}")
        if self.debug:
            logger.info(statement, caller=self)
        res = self.cursor.execute(statement)
        if self.auto_commit:
    ...
    def update(self, entry, table, cond):
        ...
        statement = str.format(f"UPDATE {table} SET {values} WHERE {conds}")
        if self.debug:
            logger.info(statement, caller=self)
        res = self.cursor.execute(statement)
        if self.auto_commit:
